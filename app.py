...
# - Opciones para generar también PDF (pagarés manuales, desde Excel y convenio grupal).

import io, os, re, zipfile, tempfile, sys, subprocess, shutil
...
# --- DOCX -> PDF helpers ---
def _docx_to_pdf_via_docx2pdf(in_path: str, out_path: str) -> bool:
    """
    Usa docx2pdf si está disponible (Windows/Mac).
...
def _docx_to_pdf_via_libreoffice(in_path: str, out_dir: str) -> str | None:
    """
    Usa LibreOffice en modo headless si está instalado (Linux/servidor).
    """
...
def docx_bytes_to_pdf_bytes(docx_bytes: bytes) -> bytes | None:
    """
    Convierte bytes .docx a bytes .pdf intentando:
      1) docx2pdf
      2) LibreOffice headless
    """
    with tempfile.TemporaryDirectory() as td:
        in_path = str(Path(td) / "input.docx")
        out_path = str(Path(td) / "output.pdf")
        Path(in_path).write_bytes(docx_bytes)

        # 1) docx2pdf
        if _docx_to_pdf_via_docx2pdf(in_path, out_path):
...
with tab2:
    ...
    if df.empty:
        ...
        if not grupos_kgrupal:
            ...
            # Opción PDF para convenio + pagarés del grupo
            gen_pdf_grupo = st.checkbox("📄 Incluir versiones PDF (convenio y pagarés individuales)", value=False, key=f"gen_pdf_grupo_{gidx}")
